...
async def create_note(
    ...
    result = await _mongo.db.notes.insert_one(doc)
...
async def update_note(note_id: str, user_id: int, **fields: Any) -> bool:
    ...
    result = await _mongo.db.notes.update_one(
...
async def delete_note(note_id: str, user_id: int) -> bool:
    ...
    if oid is None:
        return False
    result = await _mongo.db.notes.delete_one({"_id": oid, "user_id": user_id})
...
async def get_note_by_id(
    ...
    if oid is None:
        return None
    return await _mongo.db.notes.find_one({"_id": oid, "user_id": user_id})
...
async def get_user_notes(
    ...
    if _mongo.db is None:
        return []
    cursor = (
        _mongo.db.notes.find({"user_id": user_id})
...
async def count_user_notes(user_id: int) -> int:
    if _mongo.db is None:
        return 0
    return await _mongo.db.notes.count_documents({"user_id": user_id})


async def count_total_notes() -> int:
    if _mongo.db is None:
        return 0
    return await _mongo.db.notes.count_documents({})
